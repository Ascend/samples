...
APP_ERROR DvppCommon::init(void)
{
    ...
    if (ret != 0) {
        LogFatal << "Failed to create dvpp channel: " << get_app_errcodeinfo(ret) << ".";
                                           ...
                                           VpcProcessType processType)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "CombineResizeProcess cannot be called by the DvppCommon object.";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
...
APP_ERROR DvppCommon::VpcCrop(const DvppCropInputInfo &cropInput, const DvppDataInfo &output, bool withSynchronize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "VpcCrop cannot be called by the DvppCommon object";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
...
APP_ERROR DvppCommon::CombineCropProcess(DvppCropInputInfo &input, DvppDataInfo &output, bool withSynchronize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "CombineCropProcess cannot be called by the DvppCommon object.";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
...
APP_ERROR DvppCommon::JpegDecode(DvppDataInfo &input, DvppDataInfo &output, bool withSynchronize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "JpegDecode cannot be called by the DvppCommon object.";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
...
APP_ERROR DvppCommon::CombineJpegdProcess(const RawData& imageInfo, acldvppPixelFormat format, bool withSynchronize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "CombineJpegdProcess cannot be called by the DvppCommon object.";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
...
APP_ERROR DvppCommon::CombineVdecProcess(const std::shared_ptr<DvppDataInfo>& data, void *userData)
{
    ...
    if (!isVdec_) {
        LogError << "CombineVdecProcess cannot be called by the DvppCommon object.";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
    // create stream desc
                                 ...
                                 bool withSynchronize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "JpegEncode cannot be called by the DvppCommon object.";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
    ...
    ret = acldvppJpegEncodeAsync(dvppChannelDesc_, encodeInputDesc_.get(), 
        output.data, &output.dataSize, jpegeConfig, dvppStream_);
    if (ret != APP_ERR_OK) {
...
APP_ERROR DvppCommon::GetJpegEncodeDataSize(DvppDataInfo &input, acldvppJpegeConfig *jpegeConfig, uint32_t &encSize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "GetJpegEncodeDataSize cannot be called by the DvppCommon object";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
                                          ...
                                          acldvppPixelFormat format, bool withSynchronize)
{
    // Return special error code when the DvppCommon object is initialized with InitVdec
    if (isVdec_) {
        LogError << "CombineJpegeProcess cannot be called by the DvppCommon object ";
        return APP_ERR_DVPP_OBJ_FUNC_MISMATCH;
    }
    inputImage_ = std::make_shared<DvppDataInfo>();
    inputImage_->format = format;
