...
function main() {
    ...
    atc --singleop=test_data/config/matmul_tik_op.json  --soc_version=Ascend310 --output=op_models
    if [ $? -ne 0 ];then
    ...
    # Run the program
    ./execute_matmul_tik_op > matmul_sample.log
    if [ $? -ne 0 ];then
    ...
    cd ${project_path}/run/out/test_data/data
    python3 check_out.py >> matmul_sample.log
    if [ $? -ne 0 ];then
        echo "ERROR: run failed. the result of reasoning is wrong!"
        return ${inferenceError}
    fi

    if [[ `grep "the result is correct" ${project_path}/run/out/test_data/data/matmul_sample.log` ]]; then
