...
1.  生成Conv2dTik算子的单算子离线模型文件。
    ...
    2.  在out目录下执行如下命令，生成单算子模型文件。
        ...
        **atc --singleop=test\_data/config/conv2d\_tik\_op.json  --soc\_version=*Ascend310* --output=op\_models**
        ...
        在当前目录的op\_models目录下生成单算子的模型文件**0\_Conv2DTik\_1\_0\_8\_256\_7\_7\_1\_0\_256\_256\_3\_3\_1\_0\_8\_256\_7\_7.om**，命名规范为：序号+opType + 输入的描述\(dateType\_format\_shape\)+输出的描述。
...
4.  在硬件设备的Host侧执行单算子验证文件。
    1.  以运行用户（例如HwHiAiUser）拷贝开发环境中样例工程acl\_execute\_conv2d/run/目录下的out文件夹到运行环境任一目录，例如上传到/home/HwHiAiUser/HIAI\_PROJECTS/run\_conv2d/目录下。
