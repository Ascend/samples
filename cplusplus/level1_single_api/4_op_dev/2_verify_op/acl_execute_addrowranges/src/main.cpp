...
OperatorDesc CreateOpDesc()
{
    ...
    std::vector<int64_t> shape2{8, 2};
    std::string opType = "AddRowRanges";
    aclDataType dataType1 = ACL_FLOAT32;
    aclDataType dataType2 = ACL_INT32;
    aclFormat format = ACL_FORMAT_ND;
    OperatorDesc opDesc(opType);
    opDesc.AddInputTensorDesc(dataType1, shape1.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dataType1, shape1.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dataType2, shape2.size(), shape.data(), format);
    opDesc.AddOutputTensorDesc(dataType1, shape1.size(), shape.data(), format);
    return opDesc;
}
