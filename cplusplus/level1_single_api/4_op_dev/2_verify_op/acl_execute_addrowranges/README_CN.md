...
1.  生成AddRowRanges算子的单算子离线模型文件。
    ...
    2.  在out目录下执行如下命令，生成单算子模型文件。
        ...
        **atc --singleop=test\_data/config/addrowranges\_op.json  --soc\_version=_$\{soc\_version\}_  --output=op\_models**
        ...
        dataType以及format对应枚举值请从ATC组件所在目录下的“compiler/include/graph/types.h”文件中查看，枚举值从0开始依次递增。
...
4.  在硬件设备的Host侧执行单算子验证文件。
    1.  以运行用户（例如HwHiAiUser）拷贝开发环境中样例工程acl\_execute\_addrowranges/run/目录下的out文件夹到运行环境任一目录，例如上传到/home/HwHiAiUser/HIAI\_PROJECTS/run\_addrowranges/目录下。
