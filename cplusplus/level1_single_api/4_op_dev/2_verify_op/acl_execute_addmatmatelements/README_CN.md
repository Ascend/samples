...
1.  生成Add算子的单算子离线模型文件。
    ...
    2.  在out目录下执行如下命令，生成单算子模型文件。
        ...
        **atc --singleop=test\_data/config/addmatmatelements\_op.json  --soc\_version=Ascend310 --output=op\_models**
...
3.  编译样例工程，生成单算子验证可执行文件。
    1.  切换到样例工程根目录acl\_execute\_addmatmatelements，然后在样例工程根目录下执行如下命令创建目录用于存放编译文件，例如，创建的目录为“build/intermediates/host“。
...
4.  在运行环境上执行单算子验证文件。
    1.  以运行用户（例如HwHiAiUser）拷贝开发环境中样例工程acl\_execute\_addmatmatelements/run/目录下的out文件夹到板端环境任一目录，例如上传到/home/HwHiAiUser/HIAI\_PROJECTS/run\_add/目录下。
    2.  在板端环境中执行execute\_addmatmatelements\_op文件，验证单算子模型文件。
        ...
        **./execute\_addmatmatelements\_op**
        ...
        可见输出结果=输入数据1+输入数据2，AddMatMatElements算子验证结果正确。
