...
{
    // define operator
    std::vector<int64_t> shape1{3, 32};
    std::vector<int64_t> shape2{1};
    std::string opType = "AddMatMatElements";
    aclDataType dataType = ACL_FLOAT32;
    aclFormat format = ACL_FORMAT_ND;
    OperatorDesc opDesc(opType);
    opDesc.AddInputTensorDesc(dataType, shape1.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dataType, shape1.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dataType, shape1.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dataType, shape2.size(), shape.data(), format);
    opDesc.AddInputTensorDesc(dataType, shape2.size(), shape.data(), format);
    opDesc.AddOutputTensorDesc(dataType, shape1.size(), shape.data(), format);
    return opDesc;
}
