...
void memcpyh2d_usage(char *sPrgNm)
{
 ...
 --number_of_cycles 1 --device_id 0 --memory_size 10485760 --memory_reuse 0\n");
    SAMPLE_PRT("\t\n");
...
int32_t main(int32_t argc, char *argv[])
{
    ...
    while (g_number_of_cycles - cycles_time != 0)
    {
        ...
        if (g_memory_reuse) {
            ...
            // 申请device内存
            aclRet = acldvppMalloc(&device_buffer, g_memory_size);
            if (aclRet != ACL_SUCCESS) {
                SAMPLE_PRT("acldvppMalloc failed\n");
                continue;
            }
        ...
        while (g_release_cycle - release_cycle_time != 0)
        {
            ...
            // 初始化内存，构建全7的数据
            aclRet = aclrtMemset(host_buffer, g_memory_size, 7, g_memory_size);
            if (aclRet != ACL_SUCCESS) {
                SAMPLE_PRT("aclrtMemset failed\n");
                continue;
            }
            // 查看初始化后的内存
            print_data("At first memset host data", host_buffer, g_memory_size, ACL_MEMCPY_HOST_TO_HOST);
            // 内存拷贝H2D
            aclRet = aclrtMemcpy(device_buffer, g_memory_size, host_buffer, g_memory_size, ACL_MEMCPY_HOST_TO_DEVICE);
            if (aclRet != ACL_SUCCESS) {
                SAMPLE_PRT("aclrtMemcpy failed\n");
                continue;
            }
        ...
        for (auto host_buffer : host_mem_list) {
            aclRet = aclrtFreeHost(host_buffer);
            if (aclRet != ACL_SUCCESS) {
        ...
        for (auto device_buffer : dev_mem_list) {
            aclRet = acldvppFree(device_buffer);
            if (aclRet != ACL_SUCCESS) {
                SAMPLE_PRT("acldvppFree failed\n");
                continue;
            }
        }
        // 调用内存查询接口，查询divice内存
