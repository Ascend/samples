...
function downloadDataWithVerifySource() {
    ...
    wget -O ${project_path}/verify_image/"dvpp_output.yuv" ${verify_source}"dvpp_output.yuv" --no-check-certificate
    if [ $? -ne 0 ];then
...
function main() {
    ...
    result=$(ls ${project_path}/out/output/dvpp_output.yuv 2>/dev/null)
    if [[ ${result}"x" = "x" ]];then
