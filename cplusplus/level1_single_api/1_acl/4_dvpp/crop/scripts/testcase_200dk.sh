...
data_source="https://c7xcode.obs.cn-north-4.myhuaweicloud.com/models/crop/test_image/"
verify_source="https://c7xcode.obs.cn-north-4.myhuaweicloud.com/models/crop/verify_source/"
...
function main() {
    ...
    # 运行程序
    ./main ${project_path}/data/wood_rabbit_1024_1068_nv12.yuv 1024 1068 ${project_path}/out/output/output.yuv 350 280 200 200
    if [ $? -ne 0 ];then
    ...
    for outimage in $(find ${project_path}/verify_image -name "*.yuv");do
        tmp=`basename $outimage`
        if [[ ! -d "${project_path}/out/output" ]];then
            echo "ERROR: not find results folders!"
            return ${verifyResError}
        fi
        for test_file in `find ${project_path}/out/output -name "*yuv"`;do
