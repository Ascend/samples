...
int32_t sample_comm_vpc_crop_resize_paste(FuncInput funcInput)
{
    ...
    // start to call hi_mpi_vpc_crop_resize_paste interface
    uint32_t taskID = 0;
    ret = hi_mpi_vpc_crop_resize_paste(chnId, &inputPic, cropResizePasteInfos, multiCount, &taskID, -1);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_vpc_crop_resize_paste failed, ret = %#x!\n", ret);
        dvpp_mem_free(inputPic.picture_address);
        inputPic.picture_address = nullptr;
        for (uint32_t i = 0; i < multiCount; i++) {
    ...
    // write the first picture in the cropResizePasteRegionInfos array
    uint32_t picIndex = 1;
    ret = handle_output_data(cropResizePasteInfos[picIndex - 1].dest_pic_info, dstBufferSize[picIndex - 1],
        outputFileName);
    if (ret != HI_SUCCESS) {
