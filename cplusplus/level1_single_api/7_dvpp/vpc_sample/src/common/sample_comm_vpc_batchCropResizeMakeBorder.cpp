...
int32_t sample_comm_vpc_batch_crop_resize_make_border(FuncInput funcInput)
{
    ...
    // start to call hi_mpi_vpc_batch_crop_resize_make_border interface
    uint32_t taskID = 0;
    ret = hi_mpi_vpc_batch_crop_resize_make_border(chnId, (const hi_vpc_pic_info**)batchInputPic, srcPicNum,
        cropResizeMakeBorderInfos, regionCount, &taskID, -1);
    if (ret != HI_SUCCESS) {
        SAMPLE_PRT("hi_mpi_vpc_batch_crop_resize_make_border failed, ret = %#x!\n", ret);
        for (uint32_t j = 0; j < srcPicNum; j++) {
            dvpp_mem_free(batchInputPic[j]->picture_address);
            batchInputPic[j]->picture_address = nullptr;
            free(batchInputPic[j]);
    ...
    for (uint32_t j = 0; j < totalRegionCount; j++) {
        ...
        ret = handle_output_data(cropResizeMakeBorderInfos[j].dest_pic_info, dstBufferSize[j],
