...
function main() {
    ...
    if [[ $(find ${HOME}/models/${project_name} -name ${model_name}".om")"x" = "x" ]];then 
        ...
        #atc --model=${project_path}/tf_model/${tf_model##*/} - --framework=3 --output=${HOME}/models/${project_name}/${model_name} --soc_version=Ascend310 --insert_op_conf=${project_path}/tf_model/aipp.cfg
        if [ $? -ne 0 ];then
