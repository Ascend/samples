...
data_source1="https://c7xcode.obs.cn-north-4.myhuaweicloud.com/models/vdecandvenc/out_video.h264"
data_source2="https://c7xcode.obs.cn-north-4.myhuaweicloud.com/models/vdecandvenc/person.mp4"
...
function downloadData() {
    ...
    wget -O ${project_path}/data/"out_video.h264"  ${data_source1}  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download test1.jpg failed, please check Network."
        return 1
    fi

    wget -O ${project_path}/data/"person.mp4"  ${data_source2}  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download test1.jpg failed, please check Network."
        return 1
    fi
...
function main() {
    ...
    if [[ $(find ${HOME}/models/${project_name} -name ${model_name}".om")"x" = "x" ]];then 
        ...
        atc --model=${project_path}/model/${caffe_prototxt##*/} --weight=${project_path}/model/${caffe_model##*/} --framework=0 --output=${HOME}/models/${project_name}/${model_name} --soc_version=Ascend310 --insert_op_conf=${project_path}/model/${aipp_cfg##*/}  --output_type=FP32 --input_shape="data:1,3,300,300"  --input_format=NCHW   --save_original_model=false 
        if [ $? -ne 0 ];then
