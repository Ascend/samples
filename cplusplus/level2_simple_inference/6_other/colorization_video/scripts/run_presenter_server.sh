...
function check_board_ip()
{
    ...
    board_ip=$(cat ${app_script_path}/colorization.conf | grep "presenter_agent_ip" | awk -F'[ =]+' '{print $2}')
	board_ip=$(echo $board_ip | sed -e 's/\r//' | sed -e 's/\n//' | sed -e 's/ //')
    if [[ "$board_ip" = "" ]];then
        echo "please check your colorization.conf to make sure that each parameter has a value"
        return 1
    fi
...
function parse_presenter_view_ip()
{
    ...
    presenter_view_ip=$(cat ${app_script_path}/colorization.conf | grep "presenter_view_ip" | awk -F'[ =]+' '{print $2}')
...
function main()
{
    ...
    sed -i "s/presenter_server_ip=[0-9.]*/presenter_server_ip=${presenter_atlasdk_ip}/g" ${app_path}/presenterserver/colorization/config/config.conf
    
    echo "Use ${presenter_view_ip} to show information in browser..."
    sed -i "s/web_server_ip=[0-9.]*/web_server_ip=${presenter_view_ip}/g" ${app_path}/presenterserver/colorization/config/config.conf
