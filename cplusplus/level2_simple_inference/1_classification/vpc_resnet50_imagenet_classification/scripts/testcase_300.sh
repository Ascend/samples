...
project_name="cplusplus_vpc_resnet50_imagenet_classification"
...
function setBuildEnv() {
    if [[ ${version} = "c73" ]] || [[ ${version} = "C73" ]];then
        ...
        export NPU_HOST_LIB=${DDK_PATH}/runtime/lib64/stub
    elif [[ ${version} = "76" ]] || [[ ${version} = "c76" ]];then
        export DDK_PATH=/home/HwHiAiUser/Ascend/ascend-toolkit/latest/x86_64-linux
        export NPU_HOST_LIB=${DDK_PATH}/runtime/lib64/stub
    fi
...
function main() {
    ...
    atc --singleop=${project_path}/out/op_models/op_list.json --soc_version=Ascend310 --output=${project_path}/out/op_models
    if [ $? -ne 0 ];then
        echo "ERROR: convert model failed"
        return ${inferenceError}
    fi
    ...
    export LD_LIBRARY_PATH=/home/HwHiAiUser/Ascend/nnrt/latest/runtime/lib64:/home/HwHiAiUser/Ascend/ascend-toolkit/latest/compiler/lib64:/home/HwHiAiUser/ascend_ddk/x86/lib:${LD_LIBRARY_PATH}
