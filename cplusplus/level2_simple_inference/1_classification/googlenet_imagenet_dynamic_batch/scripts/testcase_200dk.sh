...
function downloadDataWithVerifySource() {
    ...
    wget -O ${project_path}/data/"dog1_1024_683.bin"  ${data_source}"test1.bin"  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download test1.bin failed, please check Network."
        return 1
    fi

    wget -O ${project_path}/data/"dog2_1024_683.bin"  ${data_source}"test2.bin"  --no-check-certificate
    if [ $? -ne 0 ];then
...
function main() {
    ...
    if [[ $(find ${HOME}/models/${project_name} -name ${model_name}".om")"x" = "x" ]];then 
        ...
        atc --model=${project_path}/model/${caffe_prototxt##*/} --weight=${project_path}/model/${caffe_model##*/} --framework=0 --output=${HOME}/models/${project_name}/${model_name} --soc_version=Ascend310 --insert_op_conf=${project_path}/model/${aipp_cfg##*/} --input_shape="data:-1,3,224,224" --dynamic_batch_size="1,2" --output_type=FP32 --input_format=NCHW
        if [ $? -ne 0 ];then
    ...
    result=`diff -q ./test_out.txt ../verify_image/verify_test1.txt`
    if [[ $result != "" ]]; then
