...
function downloadDataWithVerifySource() {
    ...
    wget -O ${project_path}/data/"butterfly_GT.bmp"  ${data_source}"butterfly_GT.bmp"  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download butterfly_GT.bmp failed, please check Network."
        return 1
    fi

    mkdir -p ${project_path}/verify_image/

    wget -O ${project_path}/verify_image/"butterfly_GT_SRCNN.png" ${verify_source}"butterfly_GT_SRCNN.png" --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download butterfly_GT_SRCNN.png failed, please check Network."
        return 1
    fi
...
function main() {
    ...
    if [[ $(find ${HOME}/models/${project_name} -name ${model_name}".om")"x" = "x" ]];then 
        ...
        atc --model=${project_path}/model/${caffe_prototxt##*/} --weight=${project_path}/model/${caffe_model##*/} --framework=0 --output=${HOME}/models/${project_name}/${model_name} --soc_version=Ascend310 --input_format=NCHW --input_shape="data: 1, 1, 768, 768" --output_type=FP32
        if [ $? -ne 0 ];then
