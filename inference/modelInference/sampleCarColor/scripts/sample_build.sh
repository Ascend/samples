...
function main()
{
  ...
  ret=`find ${ModelPath} -maxdepth 1 -name color_dvpp_10batch.om 2> /dev/null`

  if [[ ${ret} ]];then
    echo "[INFO] The color_dvpp_10batch.om already exists.start buiding"
  else
    echo "[ERROR] color_dvpp_10batch.om does not exist, please follow the readme to convert the model and place it in the correct position!"
    return 1
  fi
