...
class sample_resnet_quick_start(object):
    ...
    def process_input(self, input_path, model_width, model_height):
        ...
        # data processing
        RGB_image[:, :, 0] -= min_chn_0
        RGB_image[:, :, 0] = RGB_image[:, :, 0]*var_reci_chn_0
        RGB_image[:, :, 1] -= min_chn_1
        RGB_image[:, :, 1] = RGB_image[:, :, 1]*var_reci_chn_1
        RGB_image[:, :, 2] -= min_chn_2
        RGB_image[:, :, 2] = RGB_image[:, :, 2]*var_reci_chn_2
        RGB_image = RGB_image.transpose([2, 0, 1]).copy()
        result = np.frombuffer(RGB_image.tobytes())
        images = [result]
...
if __name__ == '__main__':
    ...
    # inference
    net = sample_resnet_quick_start(device, model_path)
    net.init_resource()
