...
import com.chris.depression.theme.progressIndicatorBackground
    ...
    Surface(modifier = Modifier.supportWideScreen()) {
        Scaffold(
            content = { innerPadding ->
                ...
                SurveyStart(start = start, modifier = modifier)
            },
            bottomBar = {
                OutlinedButton(
...
@Composable
private fun SurveyStart(start: SurveyState.Start, modifier: Modifier = Modifier) {
    LazyColumn(modifier = modifier.fillMaxSize()) {
        item {
            Spacer(modifier = Modifier.height(44.dp))
            Text(
                text = stringResource(id = start.surveyTitle),
                style = MaterialTheme.typography.h3,
                modifier = Modifier.padding(horizontal = 20.dp)
            )
            Text(
                text = start.surveyStart.intro,
                style = MaterialTheme.typography.subtitle1,
                modifier = Modifier.padding(20.dp)
            )
            Text(
                text = start.surveyStart.description,
