...
from fsdet.engine import DefaultTrainer, default_argument_parser, default_setup  # Added
...
def preprocess_image(batched_inputs: Tuple[Dict[str, torch.Tensor]]):
        ...
        image = torch.zeros(0, 800, 800)
        for i in range(images[0].size(0)):
            img = images[0][i]
            img = img.expand((1, 1, img.size(0), img.size(1)))
            img = img.to(dtype=torch.float32)
            img = F.interpolate(img, size=(int(800), int(800)), mode='bilinear', align_corners=False)
            img = img[0][0]
            img = img.unsqueeze(0)
...
def export_tracing(torch_model, inputs):
    ...
    inputs = preprocess_image(inputs).tensor.to(torch.float32)  # Added
    image = inputs  # Added
    ...
    if args.format == "torchscript":
        ...
        with PathManager.open(os.path.join(args.output, "model.onnx"), "wb") as f:
            opset_version = 11 
            print(f"Using opset_version {opset_version}")
            torch.onnx.export(traceable_model, (image,), f, opset_version=opset_version, verbose=True)
