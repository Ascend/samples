...
def run_exp(cfg, configs):
    ...
    if not os.path.exists(model_path):
        ...
        # ##################### add opts ###############
        if args.num_gpus == 1:
            print("Train with 1 npu!")
            train_cmd += " --opts AMP 1 OPT_LEVEL O1 LOSS_SCALE_VALUE 128 MODEL.DEVICE npu:0"
        else:
            print(f"Training with {args.num_gpus} npus!")
            train_cmd += f" --device-ids {' '.join([str(device_id) for device_id in range(args.num_gpus)])} --opts AMP 1 OPT_LEVEL O1 LOSS_SCALE_VALUE 128"
        ##################### add opts ###############
        run_cmd(train_cmd)
    ...
    if not os.path.exists(res_path):
        ...
        ##################### add opts ###############
        if args.num_gpus == 1:
            print("Test with 1 npu!")
            test_cmd += " --opts AMP 1 OPT_LEVEL O1 LOSS_SCALE_VALUE 128 MODEL.DEVICE npu:0"
        else:
            print(f"Test with {args.num_gpus} npus!")
            test_cmd += f" --device-ids {' '.join([str(device_id) for device_id in range(args.num_gpus)])} --opts AMP 1 OPT_LEVEL O1 LOSS_SCALE_VALUE 128"
        ##################### add opts ###############
        run_cmd(test_cmd)
