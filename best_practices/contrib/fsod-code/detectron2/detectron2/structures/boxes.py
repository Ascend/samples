...
class Boxes:
    ...
    def clip(self, box_size: Tuple[int, int]) -> None:
        ...
        """
        ############################
        # inf_idx = (torch.isinf(self.tensor)).cumsum(dim=1)[:, -1] == 0
        # inf_idx = inf_idx.to("npu:0")
        # print(inf_idx.device)
        # print(self.tensor.device)
        # self.tensor = self.tensor[inf_idx, ...]
        
        # inside_idx = self.inside_box((box_size))
        # self.tensor = self.tensor[inside_idx, ...]
        assert torch.isfinite(self.tensor).all(), "Box tensor contains infinite or NaN!"
        # self.tensor = torch.ones_like(self.tensor).to("npu")
        # self.tensor[:, 2:4] += 2
