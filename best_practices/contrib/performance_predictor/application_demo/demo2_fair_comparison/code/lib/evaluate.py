...
from lib import cifar
from lib import model_builder
# from lib import training_time
  ...
  def run(self):
    ...
    while True:
      ...
      try:
        ...
        for i in self.config['intermediate_evaluations']:
          evaluations.append(float(i))
    ...
    metadata = {
        'trainable_params': int(_get_param_count(self.model_dir)),
        'total_time': float(all_time),   # includes eval and other metric time
...
def _augment_and_evaluate_impl(spec, config, model_dir, epochs_per_eval=5):
  ...
  max_steps = int(config['train_epochs'] * steps_per_epoch) #976
...
def _create_estimator(spec, config, model_dir,
  ...
  run_config = NPURunConfig(model_dir=model_dir,keep_checkpoint_max=3,save_checkpoints_secs=2**30)
  ...
  estimator = NPUEstimator(model_fn=model_builder.build_model_fn(spec, config, num_train_images),config=run_config,)
