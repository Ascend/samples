...
Function: uses the face detection model to detect faces in instant videos shot by Raspberry Pi cameras.   
Input: videos shot by cameras.  
...
| Hardware| Atlas200DK  | The camera samples support only the Atlas 200 DK. For details about the product description, see the [hardware platform](https://www.hiascend.com/en/hardware/product).|
...
1. Run the following commands to upload the **face_detection_camera** directory in the development environment to any directory in the operating environment, for example, **/home/HwHiAiUser**, and log in to the operating environment (host) as the running user (**HwHiAiUser**):
   ```
