...
function check_ip()
{
    ...
    presenter_connect_ip=$(cat ${app_path}/scripts/mask_detection.conf | grep "presenter_server_ip" | awk -F'[ =]+' '{print $2}')
    ...
    presenter_view_ip=$(cat ${app_path}/scripts/mask_detection.conf | grep "presenter_view_ip" | awk -F'[ =]+' '{print $2}')
