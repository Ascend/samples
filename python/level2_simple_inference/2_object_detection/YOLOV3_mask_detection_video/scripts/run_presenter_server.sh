...
function check_board_ip()
{
    ...
    board_ip=$(cat ${app_script_path}/mask_detection.conf | grep "presenter_agent_ip" | awk -F'[ =]+' '{print $2}')
	board_ip=$(echo $board_ip | sed -e 's/\r//' | sed -e 's/\n//' | sed -e 's/ //')
    if [[ "$board_ip" = "" ]];then
        echo "please check your mask_detection.conf to make sure that each parameter has a value"
        return 1
    fi
...
function parse_presenter_view_ip()
{
    ...
    presenter_view_ip=$(cat ${app_script_path}/mask_detection.conf | grep "presenter_view_ip" | awk -F'[ =]+' '{print $2}')
