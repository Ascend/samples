tensorflow_model="https://modelzoo-train-atc.obs.cn-north-4.myhuaweicloud.com/003_Atc_Models/AE/ATC%20Model/vgg16_cat_dog/vgg16_cat_dog.pb"
...
function downloadDataWithVerifySource() {
    ...
    wget -O ${project_path}/verify_image/cat.jpg ${verify_source}"cat.jpg" --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download cat.jpg failed, please check Network."
        return 1
    fi

    wget -O ${project_path}/verify_image/dog.jpg ${verify_source}"dog.jpg" --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download dog.jpg failed, please check Network."
        return 1
    fi
...
function main() {
    ...
    if [[ $(find ${HOME}/models/${project_name} -name ${model_name}".om")"x" = "x" ]];then 
        ...
        atc --output_type=FP32 --input_shape="input_1:1,224,224,3" --input_format=NHWC --model=${project_path}/model/${tensorflow_model##*/} --insert_op_conf=${project_path}/model/${aipp_cfg##*/} --output=${HOME}/models/${project_name}/${model_name} --framework=3 --soc_version=Ascend310
        if [ $? -ne 0 ];then
