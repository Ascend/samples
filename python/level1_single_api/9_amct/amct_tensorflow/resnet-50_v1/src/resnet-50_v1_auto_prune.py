...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='ResNet-50 Prune DEMO')
    ...
    parser.add_argument(
        '--test_iter', dest='test_iter', default=1, type=int,
        help='The number of test_iteration in auto_channel_prune_search.')
...
def auto_channel_prune_search(saver, output_cfg):
    """do auto_channel_prune_search"""
    ...
    loss, _, _ = get_loss(input_2, logits)
    optimizer = tf.train.RMSPropOptimizer(ARGS.learning_rate, momentum=ARGS.momentum)
    optimizer.minimize(loss)

    # Restore checkpoint and do auto_channel_prune_search
    session = tf.Session()
    ...
    for i in range(ARGS.test_iter):
        ...
        feed_dict = {input_1: image, input_2: label}
        input_data.append(feed_dict)
    session.close()

    config_defination = ARGS.config_defination
    amct.auto_channel_prune_search(graph, [PREDICTIONS], config_defination, input_data, output_cfg)
...
def main(): # pylint: disable=R0914, R0915
    ...
    # Phase Do auto_channel_prune_search
    # Step 1: Load the training model.
    tf.reset_default_graph()
    graph = tf.get_default_graph()
    saver = tf.train.import_meta_graph(ARGS.train_model)
    # Step 2: Process auto_channel_prune_search.
    output_cfg = os.path.join(OUTPUTS, 'prune_cfg.cfg')
    auto_channel_prune_search(saver, output_cfg)

    # Phase Create and retrain a masked model
    # Step 1: Generate training dataset.
    tf.reset_default_graph()
    graph = tf.get_default_graph()
    # Step 2: Load the training model.
    saver = tf.train.import_meta_graph(ARGS.train_model)
    # Step 3: Prepare path of record file and config_defination
    record_file = os.path.join(OUTPUTS, 'record.txt')
    # Step 4: Generate the retraining model in the given graph with pruning masks inserted
    amct.create_prune_retrain_model(graph, [PREDICTIONS], record_file, output_cfg)
    # Step 5: Retrain the masked model and save trained parameters
    retrain_ckpt = os.path.join(OUTPUTS, 'resnet_v1_50_retrain')
    retrain(saver, retrain_ckpt)

    # Phase Convert the masked model
    # Step 1: Load the evaluation model.
    tf.reset_default_graph()
    graph = tf.get_default_graph()
    saver = tf.train.import_meta_graph(ARGS.eval_model)
    # Step 2: Generate the masked model again but using graph in evaluation mode
    amct.create_prune_retrain_model(graph, [PREDICTIONS], record_file, output_cfg)
    # Step 3: Set the variables which needed to restore.
    variables_to_restore = tf.global_variables()
    saver_restore = tf.train.Saver(variables_to_restore)
    # Step 4: Restore the variables
    session = tf.Session()
    session.run(tf.global_variables_initializer())
    retrain_ckpt = retrain_ckpt + '-' + str(ARGS.train_iter)
    saver_restore.restore(session, retrain_ckpt)
    # Step 5: Convert all variables to constants and then save masked model as 'pb' file
    constant_graph = tf.graph_util.convert_variables_to_constants(
        session, graph.as_graph_def(), [PREDICTIONS])
    pb_path = os.path.join(OUTPUTS, 'resnet_v1_50_masked.pb')
    with tf.io.gfile.GFile(pb_path, 'wb') as pb_file:
        pb_file.write(constant_graph.SerializeToString())
    session.close()

    # Step 6: Convert masked model to a pruned model
    pruned_pb_path = os.path.join(OUTPUTS, 'resnet_v1_50')
    amct.save_prune_retrain_model(pb_path, [PREDICTIONS], record_file, pruned_pb_path)

    # Phase Validate pruned model
    # Step 1: Generate validation dataset.
    tf.reset_default_graph()
    graph = tf.get_default_graph()

    # Step 2: Load the pruned model and validation it.
    # note that the pruned model needs to be retrained properly to reach its original accuracy
    pruned_pb_file = pruned_pb_path + '_quantized.pb'
    with tf.io.gfile.GFile(pruned_pb_file, 'rb') as fid:
