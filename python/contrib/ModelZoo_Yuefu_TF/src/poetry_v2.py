...
global max_decode_len
if os.environ.get('USE_NPU') in ['True', '1']:
    ...
    if int(rank_size) > 1:
        set_split_strategy_by_idx([12, 13])

max_decode_len = int(os.environ.get('max_decode_len'))
# coding=utf-8
...
def top_k_logits(logits, k, history_mask, last_logit):
    ...
    tensor0 = tf.constant(8024, dtype=tf.int32)
    ...
    return history_mask, tf.cond(tf.equal(tensor3, tensor2), lambda: logits, lambda: _top_k(), )
...
class PoetryBertConfig(object):
    ...
    def from_dict(cls, json_object):
        ...
        config = PoetryBertConfig(vocab_size=None, temperature=None)
...
class PoetryBertModel(object):
    ...
    def __init__(self,
        ...
        def get_predict_output(bert_config, embedding_table, sequence_output, history_mask, last_logit):
            ...
            with tf.variable_scope("cls/predictions", reuse=tf.AUTO_REUSE):
                ...
                new_history_mask, topk_output_ids = top_k_logits(log_probs, k=3, history_mask=history_mask,
        ...
        def step(input_ids, past=None, start_token_idx=None, history_mask=None, last_logit=100):
            ...
            new_history_mask, predicted_logit = get_predict_output(config, self.embedding_table, self.sequence_output,
                                                                   history_mask, last_logit)
            predicted_logit = tf.expand_dims(predicted_logit, 0)
            return predicted_logit, past, start_token_idx, new_history_mask

        def cond(start_token_idx):
            """cond"""
        ...
        def body(past, predicted_logit, start_token_idx, sequence_predict, history_mask):
            ...
            new_predicted_logit, new_past, new_start_token_idx, new_history_mask = step(predicted_logit, past=past,
                                                                                        ...
                                                                                        last_logit=predicted_logit[0][
                                                                                            0])
            new_predicted_logit = tf.cond(tf.less(start_token_idx, init_predict_token_idx - 1),
                                          lambda: init_input_ids[:, start_token_idx + 1:start_token_idx + 2],
                                          lambda: new_predicted_logit)
        ...
        self.past = None
        self.predicted_logit = tf.fill([1, 1], 101)
        self.start_token_idx = 0
        self.sequence_predict = tf.fill([1, 1], 101)
        self.history_mask = tf.fill([1, 21128], 1.0)
        for _ in range(max_decode_len - 1):
            self.past, self.predicted_logit, self.start_token_idx, self.sequence_predict, self.history_mask = body(
                self.past, self.predicted_logit, self.start_token_idx, self.sequence_predict, self.history_mask)
            print(self.sequence_predict)
    ...
    def get_predicted_tokens(self):
        """get_predicted_tokens"""
        return self.sequence_predict
...
def gelu(input_tensor):
    ...
    # return npu_unary_ops.gelu(input_tensor)
...
class PoetryModelDemo(object):
    """PoetryModelDemo"""
    def __init__(self,
                 seed=None,
                 nsamples=1,
                 batch_size=1,
                 vocab_file="../models/gvocab.txt",
                 do_lower_case=True,
                 length=None,
                 temperature=1.0,
        ...
        with open(black_list_file) as f:
            self.black_list = [ele.strip() for ele in f.readlines()]
        with open(white_list_file) as f:
            self.white_list = [ele.strip() for ele in f.readlines()]

        if batch_size is None:
            batch_size = 1
        assert nsamples % batch_size == 0
        self.bert_config = PoetryBertConfig(vocab_size=21128, temperature=temperature)
        self.input_ids = tf.placeholder(tf.int32, [batch_size, max_decode_len])
        self.init_predict_token_idx = tf.placeholder(tf.int32, [])

        self.model = PoetryBertModel(config=self.bert_config, is_training=False, init_input_ids=self.input_ids,
                                     init_predict_token_idx=self.init_predict_token_idx)
        np.random.seed(seed)
        ...
        if os.environ.get('USE_NPU') in ['True', '1']:
            ...
            custom_op.parameter_map["enable_auto_mix_precision"].b = True
    ...
    def generate_text_onestyle(self, raw_text, poetry_style):
        """generate_text_onestyle"""
        original_text = poetry_style + "(格式)" + raw_text + "(诗歌标题)"
        for b_char in self.black_list:
            raw_text = raw_text.replace(b_char, random.choice(self.white_list))
        raw_text = poetry_style + "(格式)" + raw_text + "(诗歌标题)"
        context_tokens = PoetryBertModel.encodetext(raw_text, vocab_file=self.vocab_file,
                                                    do_lower_case=self.do_lower_case)
        input_context_tokens = [101]
        # print (input_context_tokens)
        input_context_tokens.extend(context_tokens[-100:])
        # print (input_context_tokens)
        input_paddings = [0] * max_decode_len
        input_length = len(input_context_tokens)
        print(input_length)
        # print (input_context_tokens)
        input_context_tokens.extend(input_paddings)
        # print (input_context_tokens)
        input_context_tokens = input_context_tokens[:max_decode_len]
        print(input_context_tokens)
        for _ in range(self.nsamples // self.batch_size):
            out = self.sess.run(self.output, feed_dict={
                self.input_ids: [input_context_tokens for _ in range(self.batch_size)],
                self.init_predict_token_idx: input_length - 1
            })[0]
            # context_tokens.extend(out)
            print(out)
            context_tokens = out[input_length:]
        print(context_tokens)
        output_string = (''.join(
            PoetryBertModel.decodetext(context_tokens, vocab_file=self.vocab_file, do_lower_case=self.do_lower_case)))
        print(output_string)
        generation_string = output_string.replace(raw_text, '')
        print("generation_string", generation_string)
...
def add_arguments(parser):
    ...
    # network
    parser.add_argument("--title", type=str, default="", help="Input poetry title")
...
if __name__ == '__main__':
    ...
    title = FLAGS.title
    poetry_type = FLAGS.type
    # print(demo.generate_text("空谷幽兰"))
    print(demo.generate_text_onestyle(title, poetry_type))
