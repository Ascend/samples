...
do {if (params_.is_debug){printf(fmt, ##__VA_ARGS__);}}while(0)
...
bool AclEngine::ParseParams(int argc, char **argv) {
  const struct option long_options[] = {
    ...
    {"outputFile", 1, nullptr, 'o'},
    {"model", 1, nullptr, 'm'},
    {"batchSize", 1, nullptr, 'b'},
    {"loopNum", 1, nullptr, 'l'},
    {"help", 0, nullptr, 'h'},
    {"onlyOneBatch", 0, nullptr, 'x'},
    {"debug", 0, nullptr, 'd'},
    {nullptr, 0, nullptr, 0},
  };
  ...
  while (true) {
    int option_index = 0;
    int c = getopt_long(argc, argv, "i:o:m:b:l:h", long_options, &option_index);
    if (c == -1) {
      break;
    }

    switch (c) {
    case 'i': split(optarg, &(params_.input_files), ",");
      printf("[INFO]inputFile NUM = %ld\n", params_.input_files.size());
      break;
    case 'o': params_.output_path = std::string(optarg);
      break;
    case 'm': params_.model_file = std::string(optarg);
      break;
    case 'b': params_.batch_size = strtol(optarg, nullptr, 10);
      break;
    case 'l': params_.loops = strtol(optarg, nullptr, 10);
      break;
    case 'h': params_.is_help = true;
      break;
    case 'x': params_.only_run_one_batch = true;
      break;
    case 'd': params_.is_debug = true;
      break;
    default: parse_result = false;
      break;
    }
  }
...
bool AclEngine::Inference() {
  ...
  if (input_count_ != params_.input_files.size()) {
    printf("[ERROR] input file num not match, [%ld / %ld]\n",
  ...
  auto all_files_count = all_inputs[0].size(); //一共多少个文件，一次输入当作一个文件
  printf("all_files_count=%lu\n", all_files_count);
  ...
  for (size_t index = 0; index < all_files_count; index++) {
    if (index < params_.batch_size * batch_index) {
      for (size_t input_index = 0; input_index < input_count_; input_index++) {
        inferFileVec[input_index].push_back(all_inputs[input_index][index]);
      }
    }
    if (index + 1 == all_files_count ||
      ...
      retVal = AclInferenceProcess(model_id_,
                                   batch_dsts_, inferFileVec,
                                   &cost_times);
...
aclError AclEngine::InitAclDeviceContext() {
  ...
  for (uint32_t devIndex = device_start_id;
       devIndex < kDeviceNum + device_start_id;
       devIndex++) {
...
void AclEngine::DestroyAclModelDesc(uint32_t modelId,
...
void AclEngine::DestroyAclDeviceContext(uint32_t devNum,
...
void AclEngine::ReleaseAllAclModelResource(uint32_t modelId,
    aclmdlDesc *modelDesc,
...
int AclEngine::LoadModel() {
  ...
  int ret = aclmdlQuerySizeFromMem(model_data_ptr_,
                                   model_size, &mem_size_,
                                   &weight_size_);
  if (ret != 0) {
    printf("model aclmdlQuerySizeFromMem failed, ret[%d]\n", ret);
    DestroyAclDeviceContext(kDeviceNum, contexts_);
  } else {
    printf("aclmdlQuerySizeFromMem success, kMemSize[%lu], kWeightSize[%lu]\n",
           mem_size_,
           weight_size_);
  }
  ...
  model_desc_ptr_ = aclModelLoadAndIOGet(model_data_ptr_, model_size, &model_id_,
                                         device_ptr_, mem_size_, weight_ptr_,
                                         weight_size_);
...
aclmdlDesc *AclEngine::aclModelLoadAndIOGet(const void *model,
    ...
    void *weight_ptr,
    size_t weight_size) {
  aclError ret = aclmdlLoadFromMemWithMem(model,
                                          model_size,
                                          model_id,
                                          dev_ptr,
                                          mem_size,
...
int AclEngine::ReadInputFiles(const vector<vector<string>> &inputs) {
  ...
  for (size_t inIdx = 0; inIdx < input_count_; ++inIdx) {
    ...
    printf("fileSize = %d \n", (int)fileSize);
    size_t pos = 0;
    bool isInputBin = false; // TO READ BIN WITHOUT C++ PREPROCESS OR READ TXT WITH C++ PREPROCESS
    ...
    for (const auto& fileName : fileNames) {
      if (isInputBin == true) {
        ...
        if (fread(((char*)p_imgBuf) + pos, sizeof(char), fileSize, pFile) != fileSize) {
          printf("fread %s failed. file size not match input size %ld.\n",
        ...
        cout << "start to do PREPROCESS ........................" << endl;
        PreProcess(fileName, p_imgBuf, pos, fileSize); // data preprocess
    ...
    void *pbatchDst;
    auto ret = aclrtMalloc(&pbatchDst, fileSize, ACL_MEM_MALLOC_NORMAL_ONLY);
    if (ret != ACL_ERROR_NONE) {
...
int AclEngine::AclInferenceProcess(uint32_t model_id,
  ...
  for (size_t inIdx = 0; inIdx < inputSize; ++inIdx) {
    ...
    aclDataBuffer *inputData = aclCreateDataBuffer(inputBuf[inIdx],
                               buffer_size);
  ...
  /*
  size_t index;
  ret = aclmdlGetInputIndexByName(model_desc_ptr_, ACL_DYNAMIC_TENSOR_NAME, &index);
  if (ret != ACL_ERROR_NONE) {
    printf("aclmdlGetInputIndexByName failed, maybe static model\n");
  }
  ...
  */
  ...
  for (uint32_t i = 0; i < loop_num; ++i) {
    ...
    cost_time = chrono::duration_cast<chrono::microseconds>(end - start)
                .count();
  ...
  for (size_t outIndex = 0; outIndex < output_buffer_count; ++outIndex) {
    aclDataBuffer *dataBuffer = aclmdlGetDatasetBuffer(output.get(), outIndex);
    void *data = aclGetDataBufferAddr(dataBuffer);
    ...
    for (const auto& input_file : inputs[0]) {
      ...
      outputBinFileNameIdx += input_file.substr(input_file.find_last_of('/') + 1);
      ...
      size_t len1 = fwrite(((char*)outHostData) + pos, sizeof(char), len, fop);
      if (len1 != len) {
...
void AclEngine::split(const std::string &s,
...
void AclEngine::PreProcess(const std::string& fileName, void* &p_imgBuf, size_t& pos, size_t& fileSize) {
  string one_line;
  ...
  while (getline(fcin, one_line)) {
    ...
    int n = words.size();
    while (!(words[n - 1] == "。" || words[n - 1] == "!" || words[n - 1] == "?")) {
      words.pop_back();
      n--;
    }
  ...
  string wordVocabFileName = "../tf_total_sentiment/snapshots/word_vocab.json";
