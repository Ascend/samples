...
if __name__ == "__main__":
    ...
    for picture_name in input_dir:
        # read image
        image_path = os.path.join(INPUT_DIR, picture_name)
        cv2_bgr_img = cv2.imread(image_path)
        # preprocess image
        input_image, image_w, image_h = preprocess_cv2(cv2_bgr_img)
        # model inference
        model_output = yolov3_model.execute([input_image, ])
        # post process
        result = post_process(model_output, image_w, image_h)

        print("result = ", result)
        for class_id in range(len(result['detection_classes'])):
            box = result['detection_boxes'][class_id]
            class_name = result['detection_classes'][class_id]
            confidence = result['detection_scores'][class_id]
            cv2.rectangle(cv2_bgr_img, (int(box[1]), int(box[0])), (int(box[3]), int(box[2])), COLORS[class_id % 6], 2)
            position = (max(int(box[1]), 15), max(int(box[0]), 15))
            out_label = class_name
            cv2.putText(cv2_bgr_img, out_label, position, cv2.FONT_ITALIC, 0.6, COLORS[class_id % 6], 1)
        output_file = os.path.join(OUTPUT_DIR, picture_name)
        print("output:%s" % output_file)
        cv2.imwrite(output_file, cv2_bgr_img)
