...
config_file="object_detection.conf"
...
function check_board_ip()
{
    ...
    board_ip=$(cat ${app_script_path}/${config_file} | grep "presenter_agent_ip" | awk -F'[ =]+' '{print $2}')
    echo "presenter agent ip ${board_ip}"
	board_ip=$(echo $board_ip | sed -e 's/\r//' | sed -e 's/\n//' | sed -e 's/ //')
    if [[ "$board_ip" = "" ]];then
        echo "please check your ${config_file} to make sure that each parameter has a value"
        return 1
    fi
...
function parse_presenter_view_ip()
{
    ...
    presenter_view_ip=$(cat ${app_script_path}/${config_file} | grep "presenter_view_ip" | awk -F'[ =]+' '{print $2}')
