...
function check_board_ip()
{
    ...
    board_ip=$(cat ${app_script_path}/../data/face_detection.conf | grep "presenter_agent_ip" | awk -F'[ =]+' '{print $2}')
...
function parse_presenter_view_ip()
{
    ...
    presenter_view_ip=$(cat ${app_script_path}/../data/face_detection.conf | grep "presenter_view_ip" | awk -F'[ =]+' '{print $2}')
