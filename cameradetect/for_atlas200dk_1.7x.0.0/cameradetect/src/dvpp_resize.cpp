...
DvppResize::DvppResize(aclrtStream& stream, acldvppChannelDesc *dvppChannelDesc, Resolution& size)
: stream_(stream), dvppChannelDesc_(dvppChannelDesc), resizeConfig_(nullptr),  vpcInputDesc_(nullptr),  
...
int DvppResize::InitResizeInputDesc(ImageData* inputImage)
{
    ...
    if (vpcInputDesc_ == nullptr) {
        ASC_LOG_ERROR("acldvppCreatePicDesc vpcInputDesc_ failed");
        return STATUS_ERROR;
    }
//    printf("input image width %d, height %d, al w %d h %d\n", inputImage->width, inputImage->height, alignWidth, alignHeight);
//    printf("imput size %d, inputBufferSize %d\n", inputImage->size, inputBufferSize);
...
int DvppResize::InitResizeOutputDesc()
{
    ...
    if (resizeOutWidthStride == 0 || resizeOutHeightStride == 0) {
        ASC_LOG_ERROR("InitResizeOutputDesc AlignmentHelper failed");
        return STATUS_ERROR;
    }
    ...
    if (aclRet != ACL_ERROR_NONE) {
        ASC_LOG_ERROR("acldvppMalloc vpcOutBufferDev_ failed, aclRet = %d", aclRet);
        return STATUS_ERROR;
    }

    vpcOutputDesc_ = acldvppCreatePicDesc();
    if (vpcOutputDesc_ == nullptr) {
        ASC_LOG_ERROR("acldvppCreatePicDesc vpcOutputDesc_ failed");
        return STATUS_ERROR;
    }
...
int DvppResize::InitResizeResource(ImageData* inputImage) {
    format_ = static_cast<acldvppPixelFormat>(inputImage->format);
    resizeConfig_ = acldvppCreateResizeConfig();
    if (resizeConfig_ == nullptr) {
        ASC_LOG_ERROR("Dvpp resize init failed for create config failed");
        return STATUS_ERROR;
    }

    if (STATUS_OK != InitResizeInputDesc(inputImage)) {
        ASC_LOG_ERROR("InitResizeInputDesc failed");
        return STATUS_ERROR;
    } 

    if (STATUS_OK != InitResizeOutputDesc()) {
        ASC_LOG_ERROR("InitResizeOutputDesc failed");
        return STATUS_ERROR;
    } 
...
{
   // ASC_LOG_INFO("Resize image, format %d, with %d, height %d", (int)format_, srcImage->width, srcImage->height);

    if (STATUS_OK != InitResizeResource(srcImage)) {
        ASC_LOG_ERROR("Dvpp resize failed for init error");
        return STATUS_ERROR;
    }
...
//ASC_LOG_INFO("Resize image, format %d, with %d, height %d", (int)format_, srcImage->width, srcImage->height);
    ...
    if (aclRet != ACL_ERROR_NONE) {
        ASC_LOG_ERROR("acldvppVpcResizeAsync failed, aclRet = %d", aclRet);
        return STATUS_ERROR;
    }

    aclRet = aclrtSynchronizeStream(stream_);
    if (aclRet != ACL_ERROR_NONE) {
        ASC_LOG_ERROR("resize aclrtSynchronizeStream failed, aclRet = %d", aclRet);
        return STATUS_ERROR;
    }

    resizedImage->isAligned = true;
    resizedImage->format = format_;
