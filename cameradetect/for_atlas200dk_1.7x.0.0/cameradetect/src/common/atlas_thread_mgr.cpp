...
void AtlasThreadMgr::ThreadEntry(void* arg){
    ...
    if (userInstance == nullptr) {
        ASC_LOG_ERROR("Atlas thread exit for user thread instance is null");
        return;
    }

    ret = userInstance->Init();
    if (ret) {
        ASC_LOG_ERROR("Call user atlas thread init function return error %d, thread exit", ret);
    ...
    while(THREAD_EXITING != thMgr->GetStatus()) {
        ...
        if (ret) {
            ASC_LOG_ERROR("Atlas thread call user process function return error %d, thread exit", ret);
            break;
        } 
    }
